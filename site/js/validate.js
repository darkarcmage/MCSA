...
function validateContact(form) {
	var adultsString = parseInt(form.adults.value) || 0;
	var childrenString = parseInt(form.children.value) || 0;
	var youngstersString = parseInt(form.youngsters.value) || 0;
	var studentsString = parseInt(form.students.value) || 0;

	var totalPersons = adultsString + childrenString + youngstersString + studentsString;

      fail = validateContactName(form.Name.value)
			fail += validateDates(form.dateFrom.value,form.dateTo.value)
  ...
  { alert(fail);
	...
	function validateDates(startDate, endDate){
		if(startDate=="" && endDate=="")
		 ...
		 if(end < start)
		 	return "End date can't before start.\n";
		else return "";
